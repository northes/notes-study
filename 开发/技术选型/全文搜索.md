...
- 仅提供较基本的核心功能，其他均需要自行实现。（如同分数选优、数据库备份等）
...
- 开箱即用，功能强大，可自定义设置丰富
- 使用内存映射（启动时将所有数据载入内存）
- 官方支持基于 Jieba 分词的中文分词器，对中文以及异形文字、Emoji支持较好
...
- 不支持高级搜索（`-`排除，`OR`或）。仅支持空格（表`AND`）,双引号（表词组）
...
- ❓`go-redis` 库未实现 `search` 方法，需自行封装或跑原生语句
- 需要预先定义索引名与索引字段
- 需自行维护 `key name` 保证唯一 
