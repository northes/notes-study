...
```
# 测试服
192.168.0.136 xia-vpc.wx.mini.dev.login.local
192.168.0.136 xia-vpc.wx.mini.dev.game.local
192.168.0.136 xia-vpc.wx.mini.dev.game.1.local
192.168.0.136 xia-vpc.wx.mini.dev.game.2.local
192.168.0.136 xia-vpc.wx.mini.dev.chat.local
192.168.0.136 xia-vpc.wx.mini.dev.chat.1.local
192.168.0.136 xia-vpc.wx.mini.dev.rank.local
192.168.0.136 xia-vpc.wx.mini.dev.rank.1.local
```

### 后台的“服务器配置”

直接修改 game 库中的 `game_server` 表。修改完重启 login 服务
...
实际发包可能是发 x.x.1 的包，后台配置的时候需要注意

不要只用配置工具配了就算了，要进去后台的客户端管理处看清楚